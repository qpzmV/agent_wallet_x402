...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "  2. 确保 Sponsor 有 ETH: https://sepolia.etherscan.io/address/$(grep EVMSponsorAddr common/config.go | head -1 | awk -F'"' '{print $2}')"
    echo "  3. 确保用户有 USDC: https://sepolia.etherscan.io/address/$(grep EVMUserAddr common/config.go | head -1 | awk -F'"' '{print $2}')"
    echo "  4. 获取测试币:"
